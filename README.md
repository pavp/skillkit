...
| [`review-comments`](skills/workflow/review-comments/SKILL.md) | Write review comments and reply to reviewer feedback — organic teammate voice, the mechanism behind every claim, a closing question only when a real decision belongs to the other person. Consolidates, gates on confirmation; never posts. |
...
| [`smoke-verify`](skills/workflow/smoke-verify/SKILL.md) | Fastest-possible smoke verification of an app, feature, or function — climbs a cheap-first ladder (compiles → fast tests → boots → key-flow probe), stops at the first FAIL, and returns a dual-layer report (human summary + machine YAML) with a PASS/FAIL/INCONCLUSIVE verdict. Reports; never repairs. |
