...
| [`clean-trash`](skills/workflow/clean-trash/SKILL.md) | Detect debug artifacts and comment noise in a branch's diff — debug logging, breakpoints, redundant comments, dead imports — group by type and risk, gate on confirmation, and auto-clean only confirmed high-confidence noise. Comment removal uses the surprise test and a delete bar as high as the add bar; uncertainty stays report-only. Diff scope only. |
