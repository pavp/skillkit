...
| [`slice-plan`](skills/workflow/slice-plan/SKILL.md) | Forecast the changed lines of a planned implementation BEFORE code exists — itemized per-file estimates, the same >400-line budget `slice-diff` enforces after the fact. Over budget, plans slice boundaries (files and commits per slice, dependency order) and gates on confirmation; under it, states the total and steps aside. Plans only; never implements, never mutates git. |
