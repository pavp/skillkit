...
- **Causality partition** — only `pre-existing` findings move to the follow-up section (see `output-contract.md`); `introduced` and `behavior-activated` stay in the severity sections. This is the ONLY permitted move and is causality-based, not importance-based. Severity, wording, and lens are still never altered; the orchestrator never re-decides a lens's causality tag.
