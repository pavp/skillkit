...
Emit findings in the exact shape from `finding-shape.md` — rich blockquote for 🔴🟠🟡, compact one-line for 🔵. Use `Spec` as the lens name. Two Spec specifics (per that file's "Spec only" note): the parenthetical carries a `kind` tag — `(`Spec` — `<path>` · _missing | scope-creep | wrong_)` — and `Evidence` is the quoted intent line, not code. Spec is **exempt from causality classification**: emit no `introduced`/`pre-existing` tag, and your findings always stay in the normal severity sections (never the follow-up section). Intent conformance is not a code-diff defect — a `missing` requirement points at code absent from the diff, where the category is meaningless. `Why it matters` (the gap: intent vs. diff) and `Fix` are both required and separate. If the diff faithfully matches the intent, say exactly: `No findings.`
