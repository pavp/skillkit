...
Emit findings in the exact shape from `references/finding-shape.md` — rich blockquote for 🔴🟠🟡, compact one-line for 🔵. Use `Resilience` as the lens name. `Why it matters` and `Fix` are both required and separate. If clean, say exactly: `No findings.`
